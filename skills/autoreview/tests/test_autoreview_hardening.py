...
args = sys.argv[1:]
if "--version" in args or "-v" in args:
    print(os.environ.get("AUTOREVIEW_FAKE_CLAUDE_VERSION", "2.1.237 (Claude Code)"))
    raise SystemExit(0)
...
class AutoreviewHardeningTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # continue to exercise the real scanner path.
        self.scan_patch = mock.patch.dict(
            self.helper["main_impl"].__globals__,
    ...
    def test_outgoing_pack_scan_refuses_and_names_deleted_file(self) -> None:
        ...
        finding = {
            "SourceMetadata": {
                "Data": {"Filesystem": {"file": "review-pack.txt", "line": 7}}
            },
    ...
    def test_main_accepts_string_prompts_through_scan_history_and_provider(self) -> None:
        with tempfile.TemporaryDirectory(
            prefix="autoreview-string-prompt-history-test."
        ) as history_dir, self.preparation_fixture(
            "--run-log-dir",
            history_dir,
            "--log-bundle",
        ) as (_repo, sends, _stdout, _stderr):
            original_prepare = self.helper["prepare_review_prompts"]
            scans: list[str] = []

            def string_prompts(*args, **kwargs):
                return [
                    self.helper["review_prompt_text"](prompt)
                    for prompt in original_prepare(*args, **kwargs)
                ]

            with mock.patch.dict(
                self.helper["main_impl"].__globals__,
                {
                    "prepare_review_prompts": string_prompts,
            ...
            run_dirs = list((Path(history_dir) / "runs").iterdir())
            self.assertEqual(len(run_dirs), 1)
            saved_prompts = json.loads(
                (run_dirs[0] / "prompts.json").read_text(encoding="utf-8")
            )
