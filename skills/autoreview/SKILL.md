...
| **claude**   | `--safe-mode --setting-sources user --strict-mcp-config --disallowedTools mcp__*`; explicit subscription or Bedrock auth uses an empty settings source; `--strict-mcp-config` is omitted when an endpoint-managed MCP config is present (the flag is invalid there); auto-memory and filesystem/shell tools disabled; empty external workspace; WebSearch by default (`v2.1.169+`) | Claude Code [CLI reference](https://code.claude.com/docs/en/cli-reference) |
